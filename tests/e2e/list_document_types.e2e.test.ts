...
describe("list_document_types (e2e) — regression for is_empty filter", () => {
  ...
  test("is_empty: true returns only document types with document_count === 0 and a consistent count", async () => {
    ...
    }>("list_document_types", { is_empty: true });
    ...
    for (const dt of body.results) {
      assert.equal(
        dt.document_count,
        0,
        `document type ${dt.name} (id=${dt.id}) leaked through is_empty:true with count ${dt.document_count}`
      );
    }
    assert.ok(
      body.results.length >= 1,
      "expected at least one seeded empty document type"
    );
  });
});
