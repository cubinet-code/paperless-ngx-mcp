...
- **Paperless error details were dropped.** Validation errors (`{"non_field_errors": ["password not specified"]}`) and plain-text errors ("AI is required for this feature") now appear in the tool error instead of a bare "status code 400" — for every JSON API call and for version uploads; `post_document` uploads still report only the status. HTML error pages are still reduced to the status line.
