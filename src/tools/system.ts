...
export function registerSystemTools(server: McpServer, api: PaperlessAPI) {
  ...
  server.tool(
    ...
    "List background tasks with their status, type, trigger source and results. Useful for monitoring document consumption and other async operations. Use the filters to narrow the list; `limit` sets how many are returned.",
    {
      ...
      trigger_source: z.enum(["scheduled", "web_ui", "api_upload", "folder_consume", "email_consume", "system", "manual"]).optional().describe("Filter by what triggered the task"),
      ...
      limit: z.number().int().min(1).optional().describe("Max number of tasks to return (default 25)"),
    },
    Annotations.READ,
    withErrorHandling(async (args) => {
      const { limit, ...filters } = args;
      const queryString = buildQueryString({ ...filters, page_size: limit ?? 25 });
      const response = await api.request<PaginatedResponse<Task>>(
        `/tasks/?${queryString}`
      );
      return {
